...
namespace lib2
{
    ...
    protected:
        constexpr void overflow(char_type ch) override
        {
            resize_str(1);
            Traits::assign(*this->pcur(), ch);
            this->pbump(1);
        }
    private:
        std::basic_string<CharT, Traits, Allocator> buf;
        ...
        constexpr void resize_str(size_type count)
        {
            // Turn capacity into size. Make the data "real"
            const auto written {this->pcur() - this->pbeg()};
            buf.resize_and_overwrite(buf.capacity(), [&](const auto, const auto) {
    ...
    protected:
        constexpr void overflow(const char_type ch) override
        {
            resize_str(1);
            Traits::assign(*this->pcur(), ch);
            this->pbump(1);
    ...
    void swap(basic_istringstream<CharT, Traits, Alloc>& lhs, basic_istringstream<CharT, Traits, Alloc>& rhs)
    {
        lhs.swap(rhs);
    }
    ...
    export
    template<class CharT, class Traits, class Alloc>
    void swap(basic_ostringstream<CharT, Traits, Alloc>& lhs, basic_ostringstream<CharT, Traits, Alloc>& rhs)
    {
        lhs.swap(rhs);
    }
    ...
    export
    template<class CharT, class Traits, class Alloc>
    void swap(basic_stringstream<CharT, Traits, Alloc>& lhs, basic_stringstream<CharT, Traits, Alloc>& rhs)
    {
        lhs.swap(rhs);
    }
